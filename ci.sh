...
if [ "$SPEC_OK" -eq 1 ]; then
    ...
    echo "-- Phase 5e21b: the native view hands out a usable handle (#193) --"
    UI_SPEC=native_view_demo/spec_native_view_demo \
    run_server_test "$(EX_BIN native_view_demo)" \
                    "$SCRIPT_DIR/tests/run_spec.sh" native_view_demo || FAIL=$((FAIL + 1))
