...
- **Compile the Win32 backend before claiming it works**: nothing in CI RUNS Windows, so `aether_ui_win32.c` is the one file that can be edited and shipped without ever being compiled, and has been. `ci.sh` cross-compiles it (and the shared driver and system-extras sources) whenever a mingw-w64 compiler is on the box, and the CI workflow has an isolated `win32-crosscheck` job that does the same. It LINKS as well as compiles, because a Windows API whose import library is missing compiles fine and fails at link. Locally: `brew install mingw-w64` then `./ci.sh`.
